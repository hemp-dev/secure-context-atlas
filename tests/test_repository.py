...
class RepositorySmokeTest(unittest.TestCase):
    def test_generated_indexes_and_cards(self) -> None:
        ...
        subprocess.run(python + [str(root / "scripts" / "validate_advisories.py")], cwd=root, check=True)
        subprocess.run(python + [str(root / "scripts" / "build_sbom.py")], cwd=root, check=True)
        subprocess.run(python + [str(root / "scripts" / "validate_sbom.py")], cwd=root, check=True)
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual(manifest["release"], "0.7.0")
