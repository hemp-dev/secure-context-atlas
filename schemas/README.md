...
`vulnerability.schema.json` is the canonical frontmatter contract for `vulnerabilities/**/*.md`; `finding.schema.json` and `evidence.schema.json` define model output; `threat-model.schema.json` defines assets, principals, boundaries, flows, capabilities and controls; `context-pack.schema.json`, `advisory.schema.json`, `advisory-bundle.schema.json`, `provenance.schema.json`, `eval-fixture.schema.json`, `agentic-eval.schema.json`, `detector-contract.schema.json` and `sbom.schema.json` cover v0.7 release interfaces; `technique.schema.json`, `audit-check.schema.json` and `source.schema.json` cover supporting records.
