...
Release 0.7.0 adds executable Semgrep/CodeQL detector packs, live OSV/GHSA adapters with request/response provenance, deterministic SPDX SBOM and release manifest artifacts, and a reviewed agentic/MCP retrieval benchmark. `sctx pack` excludes `inventory` and `scaffolded` cards by default; use an explicit `--maturity` filter when reviewing unfinished material. Use all evaluation suites as deterministic retrieval regressions, not as proof of a model's correctness.
