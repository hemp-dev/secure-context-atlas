---
id: "vuln.secrets.info-disclosure"
title: "Sensitive information exposure through responses or metadata"
aliases: []
summary: "A response or metadata endpoint exposes data beyond the caller's need-to-know boundary through fields, identifiers, headers or provider details."
...
surfaces: ["API","error","metadata","export"]
languages: ["any"]
...
data_flow: {"sources":["request and internal object/error state"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["HTTP response, metadata or export"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Serializer returns internal fields","Error path bypasses redaction","Metadata endpoint shares cross-tenant state"]
configuration_signals: ["Configuration, default or error paths can bypass sensitive information exposure through responses or metadata.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace request and internal object/error state to HTTP response, metadata or export across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for Sensitive information exposure through responses or metadata?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for Sensitive information exposure through responses or metadata; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.info-disclosure.positive","eval.info-disclosure.negative"]
detector_refs: []
---

# Sensitive information exposure through responses or metadata

A response or metadata endpoint exposes data beyond the caller's need-to-know boundary through fields, identifiers, headers or provider details.
...
Canonical ID: `vuln.secrets.info-disclosure`; canonical ontology: `CWE-200`.
