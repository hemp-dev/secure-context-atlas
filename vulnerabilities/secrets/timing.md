---
id: "vuln.secrets.timing"
title: "Observable timing side channel"
aliases: []
summary: "Timing or resource differences reveal secret validity, account state, authorization or data-dependent behaviour across a measurable boundary."
family: "information-secrets-privacy"
canonical_cwe: "CWE-208"
related_cwe: []
capec: []
owasp_mappings: []
asvs_mappings: []
...
surfaces: ["login","token","lookup","crypto"]
languages: ["any"]
...
data_flow: {"sources":["candidate secret, identifier or query"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["response timing or resource usage"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Early-exit secret comparison","Existing and missing records take different paths","Cache/database behaviour reveals state"]
configuration_signals: ["Configuration, default or error paths can bypass observable timing side channel.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace candidate secret, identifier or query to response timing or resource usage across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for Observable timing side channel?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for Observable timing side channel; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.timing.positive","eval.timing.negative"]
detector_refs: []
---

# Observable timing side channel

Timing or resource differences reveal secret validity, account state, authorization or data-dependent behaviour across a measurable boundary.
...
Canonical ID: `vuln.secrets.timing`; canonical ontology: `CWE-208`.
