---
id: "vuln.api.webhook-replay"
title: "Webhook replay and duplicate side effects"
aliases: []
summary: "A valid callback can be replayed or delivered out of order to repeat a side effect because freshness and idempotency are not enforced together."
...
surfaces: ["webhook","queue","payment"]
languages: ["any"]
frameworks: ["framework-agnostic"]
platforms: ["api","integration","workflow"]
...
data_flow: {"sources":["webhook body, event ID and timestamp"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["payment, provisioning or state transition"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Event ID is logged but not atomically claimed","Timestamp is ignored","Retry path bypasses signature or deduplication"]
configuration_signals: ["Configuration, default or error paths can bypass webhook replay and duplicate side effects.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace webhook body, event ID and timestamp to payment, provisioning or state transition across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for Webhook replay and duplicate side effects?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for Webhook replay and duplicate side effects; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.webhook-replay.positive","eval.webhook-replay.negative"]
detector_refs: []
---

# Webhook replay and duplicate side effects

A valid callback can be replayed or delivered out of order to repeat a side effect because freshness and idempotency are not enforced together.
...
Canonical ID: `vuln.api.webhook-replay`; canonical ontology: `CWE-294`.
