---
id: "vuln.api.inventory"
title: "Incomplete API inventory and shadow endpoint exposure"
aliases: []
summary: "Undocumented, deprecated or alternate API paths remain reachable without the same authentication, authorization, rate and logging controls as the supported surface."
...
surfaces: ["REST","GraphQL","gRPC","WebSocket"]
languages: ["any"]
...
data_flow: {"sources":["route, schema, version or protocol discovery"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["shadow endpoint or deprecated handler"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Inventory is derived only from gateway config","Old version remains deployed","Internal/admin path shares public routing"]
configuration_signals: ["Configuration, default or error paths can bypass incomplete api inventory and shadow endpoint exposure.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace route, schema, version or protocol discovery to shadow endpoint or deprecated handler across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for Incomplete API inventory and shadow endpoint exposure?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for Incomplete API inventory and shadow endpoint exposure; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.inventory.positive","eval.inventory.negative"]
detector_refs: []
---

# Incomplete API inventory and shadow endpoint exposure

Undocumented, deprecated or alternate API paths remain reachable without the same authentication, authorization, rate and logging controls as the supported surface.
...
Canonical ID: `vuln.api.inventory`; canonical ontology: `CWE-200`.
