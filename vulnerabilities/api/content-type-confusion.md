---
id: "vuln.api.content-type-confusion"
title: "Content-type and parser confusion"
aliases: []
summary: "Different components interpret the same bytes as different media types or structures, bypassing validation and security policy."
family: "api-protocols"
canonical_cwe: "CWE-436"
related_cwe: []
capec: []
owasp_mappings: []
asvs_mappings: []
...
data_flow: {"sources":["request body, content-type and filename"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["parser, validator or downstream service"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Sniffing differs between proxy and application","Validation uses one parser and execution another","Multipart boundaries are handled inconsistently"]
configuration_signals: ["Configuration, default or error paths can bypass content-type and parser confusion.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace request body, content-type and filename to parser, validator or downstream service across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for Content-type and parser confusion?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for Content-type and parser confusion; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.content-type-confusion.positive","eval.content-type-confusion.negative"]
detector_refs: []
---

# Content-type and parser confusion

Different components interpret the same bytes as different media types or structures, bypassing validation and security policy.
...
Canonical ID: `vuln.api.content-type-confusion`; canonical ontology: `CWE-436`.
