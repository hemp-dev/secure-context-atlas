---
id: "vuln.browser.postmessage"
title: "Cross-window message origin and data validation failure"
aliases: []
summary: "A window or worker trusts cross-origin messages without validating sender origin, source window, schema and authorization context."
family: "browser-client"
canonical_cwe: "CWE-940"
related_cwe: []
capec: []
owasp_mappings: []
asvs_mappings: []
...
surfaces: ["browser","iframe","extension"]
languages: ["any"]
...
data_flow: {"sources":["postMessage event, iframe or extension message"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["DOM action, token use or state mutation"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Wildcard target origin","event.data used without schema validation","Source window is not bound to a session"]
configuration_signals: ["Configuration, default or error paths can bypass cross-window message origin and data validation failure.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace postMessage event, iframe or extension message to DOM action, token use or state mutation across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for Cross-window message origin and data validation failure?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for Cross-window message origin and data validation failure; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
related_vulnerabilities: []
references: ["https://cwe.mitre.org/data/definitions/940.html"]
source_provenance: ["sources/research-notes.md:normalized defensive topic","sources/manifest.yaml:mitre-cwe"]
last_reviewed: "2026-08-08"
maturity: "scaffolded"
priority: "P1"
review_status: "needs-review"
fixture_ids: ["eval.postmessage.positive","eval.postmessage.negative"]
detector_refs: []
---

# Cross-window message origin and data validation failure

A window or worker trusts cross-origin messages without validating sender origin, source window, schema and authorization context.
...
Canonical ID: `vuln.browser.postmessage`; canonical ontology: `CWE-940`.
