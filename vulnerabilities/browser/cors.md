---
id: "vuln.browser.cors"
title: "Overbroad CORS and credentialed cross-origin access"
aliases: []
summary: "A browser trust policy allows an attacker-controlled origin to read credentialed or sensitive responses."
...
data_flow: {"sources":["Origin header and credentialed browser request"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["CORS response policy and browser-readable response"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Origin reflected without exact allowlist","Credentials enabled with broad origin","Preflight and actual request policies differ"]
configuration_signals: ["Configuration, default or error paths can bypass overbroad cors and credentialed cross-origin access.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace Origin header and credentialed browser request to CORS response policy and browser-readable response across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for Overbroad CORS and credentialed cross-origin access?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for Overbroad CORS and credentialed cross-origin access; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.cors.positive","eval.cors.negative"]
detector_refs: []
---

# Overbroad CORS and credentialed cross-origin access

A browser trust policy allows an attacker-controlled origin to read credentialed or sensitive responses.
...
Canonical ID: `vuln.browser.cors`; canonical ontology: `CWE-942`.
