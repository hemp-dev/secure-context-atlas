---
id: "vuln.browser.clickjacking"
title: "UI redress and framing policy failure"
aliases: []
summary: "A sensitive page can be framed or visually overlaid so a user performs an action without seeing the intended interface."
family: "browser-client"
canonical_cwe: "CWE-1021"
related_cwe: []
capec: []
owasp_mappings: []
asvs_mappings: []
...
surfaces: ["browser","admin console","payment"]
languages: ["any"]
...
data_flow: {"sources":["browser navigation and embedding context"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["framed sensitive action or UI"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Frame policy absent or inconsistent","Sensitive action lacks re-authentication or origin signal","Legacy and modern headers disagree"]
configuration_signals: ["Configuration, default or error paths can bypass ui redress and framing policy failure.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace browser navigation and embedding context to framed sensitive action or UI across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for UI redress and framing policy failure?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for UI redress and framing policy failure; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.clickjacking.positive","eval.clickjacking.negative"]
detector_refs: []
---

# UI redress and framing policy failure

A sensitive page can be framed or visually overlaid so a user performs an action without seeing the intended interface.
...
Canonical ID: `vuln.browser.clickjacking`; canonical ontology: `CWE-1021`.
