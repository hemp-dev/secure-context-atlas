---
id: "vuln.business.idempotency"
title: "Idempotency-key scope and atomicity failure"
aliases: []
summary: "An idempotency mechanism does not uniquely and atomically bind a business action to its principal, request semantics and final result."
...
surfaces: ["payment","provisioning","API"]
languages: ["any"]
frameworks: ["framework-agnostic"]
platforms: ["api","workflow","distributed"]
...
data_flow: {"sources":["idempotency key, request body and principal"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["database write, payment or external call"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Key is global or tenantless","Request hash is not compared","Record is created after side effect"]
configuration_signals: ["Configuration, default or error paths can bypass idempotency-key scope and atomicity failure.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace idempotency key, request body and principal to database write, payment or external call across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for Idempotency-key scope and atomicity failure?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for Idempotency-key scope and atomicity failure; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.idempotency.positive","eval.idempotency.negative"]
detector_refs: []
---

# Idempotency-key scope and atomicity failure

An idempotency mechanism does not uniquely and atomically bind a business action to its principal, request semantics and final result.
...
Canonical ID: `vuln.business.idempotency`; canonical ontology: `CWE-841`.
