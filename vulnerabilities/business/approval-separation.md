---
id: "vuln.business.approval-separation"
title: "Missing maker-checker separation in approvals"
aliases: []
summary: "The same principal can initiate and approve a sensitive workflow or alter the approval evidence after the fact."
...
surfaces: ["admin","workflow","finance"]
languages: ["any"]
frameworks: ["framework-agnostic"]
platforms: ["enterprise","web","api"]
...
data_flow: {"sources":["approval request, role and workflow state"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["release, payment, access grant or policy change"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Initiator can self-approve","Delegation erases original actor","Approval is not bound to exact change digest"]
configuration_signals: ["Configuration, default or error paths can bypass missing maker-checker separation in approvals.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace approval request, role and workflow state to release, payment, access grant or policy change across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for Missing maker-checker separation in approvals?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for Missing maker-checker separation in approvals; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.approval-separation.positive","eval.approval-separation.negative"]
detector_refs: []
---

# Missing maker-checker separation in approvals

The same principal can initiate and approve a sensitive workflow or alter the approval evidence after the fact.
...
Canonical ID: `vuln.business.approval-separation`; canonical ontology: `CWE-841`.
