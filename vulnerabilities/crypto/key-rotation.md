---
id: "vuln.crypto.key-rotation"
title: "Key lifecycle and rotation failure"
aliases: []
summary: "Cryptographic keys remain active beyond their intended scope or cannot be rotated, revoked and re-encrypted without unsafe fallback."
family: "crypto-transport"
canonical_cwe: "CWE-324"
related_cwe: []
capec: []
owasp_mappings: []
asvs_mappings: []
...
surfaces: ["encryption","signing","secrets"]
languages: ["any"]
frameworks: ["framework-agnostic"]
platforms: ["cloud","api","mobile"]
...
data_flow: {"sources":["key identifier, secret reference or encrypted record"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["decrypt/sign/verify operation"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["No key version in ciphertext","Old keys remain accepted indefinitely","Rotation falls back to hardcoded material"]
configuration_signals: ["Configuration, default or error paths can bypass key lifecycle and rotation failure.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace key identifier, secret reference or encrypted record to decrypt/sign/verify operation across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for Key lifecycle and rotation failure?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for Key lifecycle and rotation failure; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
related_vulnerabilities: []
references: ["https://cwe.mitre.org/data/definitions/324.html"]
source_provenance: ["sources/research-notes.md:normalized defensive topic","sources/manifest.yaml:mitre-cwe"]
last_reviewed: "2026-08-08"
maturity: "scaffolded"
priority: "P1"
review_status: "needs-review"
fixture_ids: ["eval.key-rotation.positive","eval.key-rotation.negative"]
detector_refs: []
---

# Key lifecycle and rotation failure

Cryptographic keys remain active beyond their intended scope or cannot be rotated, revoked and re-encrypted without unsafe fallback.
...
Canonical ID: `vuln.crypto.key-rotation`; canonical ontology: `CWE-324`.
