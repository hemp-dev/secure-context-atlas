---
id: "vuln.supply.workflow-injection"
title: "Untrusted CI metadata reaches privileged workflow code"
aliases: []
summary: "Pull-request data, branch names, issue text or repository content is interpolated into a privileged CI command or action."
...
surfaces: ["CI","SCM","release"]
languages: ["any"]
frameworks: ["framework-agnostic"]
platforms: ["ci-cd","repository","cloud"]
...
data_flow: {"sources":["commit metadata, PR title, branch or changed file"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["runner command, action input or deployment"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Expression is placed in shell context","Untrusted event gets write tokens","Workflow checks out attacker-controlled code before trust split"]
configuration_signals: ["Configuration, default or error paths can bypass untrusted ci metadata reaches privileged workflow code.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace commit metadata, PR title, branch or changed file to runner command, action input or deployment across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for Untrusted CI metadata reaches privileged workflow code?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for Untrusted CI metadata reaches privileged workflow code; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.workflow-injection.positive","eval.workflow-injection.negative"]
detector_refs: []
---

# Untrusted CI metadata reaches privileged workflow code

Pull-request data, branch names, issue text or repository content is interpolated into a privileged CI command or action.
...
Canonical ID: `vuln.supply.workflow-injection`; canonical ontology: `CWE-829`.
