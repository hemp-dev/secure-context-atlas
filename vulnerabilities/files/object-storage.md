---
id: "vuln.files.object-storage"
title: "Object-storage public exposure and key-scope failure"
aliases: []
summary: "Object storage policy, signed URL or key prefix grants broader read/write/list access than the intended tenant or operation."
...
surfaces: ["S3","blob storage","CDN"]
languages: ["any"]
frameworks: ["framework-agnostic"]
platforms: ["cloud","storage","api"]
...
data_flow: {"sources":["object key, bucket policy or signed URL"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["object storage API or CDN"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Public ACL or wildcard resource","Key prefix is treated as authorization","Signed URL permits unintended method or duration"]
configuration_signals: ["Configuration, default or error paths can bypass object-storage public exposure and key-scope failure.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace object key, bucket policy or signed URL to object storage API or CDN across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for Object-storage public exposure and key-scope failure?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for Object-storage public exposure and key-scope failure; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.object-storage.positive","eval.object-storage.negative"]
detector_refs: []
---

# Object-storage public exposure and key-scope failure

Object storage policy, signed URL or key prefix grants broader read/write/list access than the intended tenant or operation.
...
Canonical ID: `vuln.files.object-storage`; canonical ontology: `CWE-732`.
