---
id: "vuln.files.include"
title: "Local or remote file inclusion through dynamic path selection"
aliases: []
summary: "A dynamic file or module selection reaches an include/loader boundary without a fixed allowlist and containment policy."
family: "files-paths-storage"
canonical_cwe: "CWE-98"
related_cwe: []
capec: []
owasp_mappings: []
asvs_mappings: []
...
surfaces: ["template","include","plugin"]
languages: ["any"]
frameworks: ["framework-agnostic"]
platforms: ["server","web","worker"]
...
data_flow: {"sources":["template name, locale, plugin or path parameter"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["include, module loader or interpreter"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Path is assembled from request data","Extension/normalization can be bypassed","Loader has network or process-capable modules"]
configuration_signals: ["Configuration, default or error paths can bypass local or remote file inclusion through dynamic path selection.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace template name, locale, plugin or path parameter to include, module loader or interpreter across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for Local or remote file inclusion through dynamic path selection?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for Local or remote file inclusion through dynamic path selection; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.include.positive","eval.include.negative"]
detector_refs: []
---

# Local or remote file inclusion through dynamic path selection

A dynamic file or module selection reaches an include/loader boundary without a fixed allowlist and containment policy.
...
Canonical ID: `vuln.files.include`; canonical ontology: `CWE-98`.
