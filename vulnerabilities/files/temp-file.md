---
id: "vuln.files.temp-file"
title: "Insecure temporary-file and workspace handling"
aliases: []
summary: "Temporary files or workspaces can be guessed, shared, retained or accessed by a different principal during a sensitive operation."
family: "files-paths-storage"
canonical_cwe: "CWE-377"
related_cwe: []
capec: []
owasp_mappings: []
asvs_mappings: []
...
surfaces: ["upload","export","build"]
languages: ["any"]
frameworks: ["framework-agnostic"]
platforms: ["server","worker","ci-cd"]
...
data_flow: {"sources":["temporary name, upload or job identifier"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["temporary filesystem path or cleanup job"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Predictable filename","Shared directory permissions","Cleanup occurs after a long delay or not at all"]
configuration_signals: ["Configuration, default or error paths can bypass insecure temporary-file and workspace handling.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace temporary name, upload or job identifier to temporary filesystem path or cleanup job across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for Insecure temporary-file and workspace handling?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for Insecure temporary-file and workspace handling; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.temp-file.positive","eval.temp-file.negative"]
detector_refs: []
---

# Insecure temporary-file and workspace handling

Temporary files or workspaces can be guessed, shared, retained or accessed by a different principal during a sensitive operation.
...
Canonical ID: `vuln.files.temp-file`; canonical ontology: `CWE-377`.
