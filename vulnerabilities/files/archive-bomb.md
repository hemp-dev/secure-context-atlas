---
id: "vuln.files.archive-bomb"
title: "Archive expansion and decompression resource exhaustion"
aliases: []
summary: "A compressed or nested archive expands beyond bounded storage, CPU or file-count budgets during validation or extraction."
family: "files-paths-storage"
canonical_cwe: "CWE-409"
related_cwe: []
capec: []
owasp_mappings: []
asvs_mappings: []
...
surfaces: ["archive","upload","worker"]
languages: ["any"]
...
data_flow: {"sources":["uploaded archive or provider-supplied package"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["decompressor, filesystem or media parser"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Compression ratio is unbounded","Nested archives are recursively extracted","File count and total size are checked after expansion"]
configuration_signals: ["Configuration, default or error paths can bypass archive expansion and decompression resource exhaustion.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace uploaded archive or provider-supplied package to decompressor, filesystem or media parser across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for Archive expansion and decompression resource exhaustion?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for Archive expansion and decompression resource exhaustion; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.archive-bomb.positive","eval.archive-bomb.negative"]
detector_refs: []
---

# Archive expansion and decompression resource exhaustion

A compressed or nested archive expands beyond bounded storage, CPU or file-count budgets during validation or extraction.
...
Canonical ID: `vuln.files.archive-bomb`; canonical ontology: `CWE-409`.
