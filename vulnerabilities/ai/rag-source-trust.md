---
id: "vuln.ai.rag-source-trust"
title: "RAG provenance and source-trust confusion"
aliases: []
summary: "Retrieved content is used for a security-sensitive decision without preserving source, tenant, freshness, authorization and trust level."
...
genai_mappings: ["LLM08:2025","ASI06"]
applies_to: ["source code","configuration","architecture"]
surfaces: ["RAG","vector database","search"]
languages: ["any"]
...
data_flow: {"sources":["document, embedding result or search filter"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["retrieval context, answer or action"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Vector metadata omits tenant/policy","Top-k result outranks system policy","Deleted or revoked source remains retrievable"]
configuration_signals: ["Configuration, default or error paths can bypass rag provenance and source-trust confusion.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace document, embedding result or search filter to retrieval context, answer or action across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for RAG provenance and source-trust confusion?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for RAG provenance and source-trust confusion; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.rag-source-trust.positive","eval.rag-source-trust.negative"]
detector_refs: []
---

# RAG provenance and source-trust confusion

Retrieved content is used for a security-sensitive decision without preserving source, tenant, freshness, authorization and trust level.
...
Canonical ID: `vuln.ai.rag-source-trust`; canonical ontology: `CWE-829`.
