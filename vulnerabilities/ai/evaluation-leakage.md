---
id: "vuln.ai.evaluation-leakage"
title: "Evaluation and feedback data leakage"
aliases: []
summary: "Evaluation prompts, hidden tests, user feedback or sensitive labels cross into model, logging or training paths where they can be disclosed or overfit."
...
genai_mappings: ["LLM02:2025","ASI09"]
applies_to: ["source code","configuration","architecture"]
surfaces: ["evaluation","feedback","training"]
languages: ["any"]
...
data_flow: {"sources":["evaluation case, user feedback or grader output"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["model provider, artifact, log or training dataset"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Hidden test data is in prompts/logs","Feedback is reused without consent or isolation","Evaluation fixture contains real data"]
configuration_signals: ["Configuration, default or error paths can bypass evaluation and feedback data leakage.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace evaluation case, user feedback or grader output to model provider, artifact, log or training dataset across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for Evaluation and feedback data leakage?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for Evaluation and feedback data leakage; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.evaluation-leakage.positive","eval.evaluation-leakage.negative"]
detector_refs: []
---

# Evaluation and feedback data leakage

Evaluation prompts, hidden tests, user feedback or sensitive labels cross into model, logging or training paths where they can be disclosed or overfit.
...
Canonical ID: `vuln.ai.evaluation-leakage`; canonical ontology: `CWE-200`.
