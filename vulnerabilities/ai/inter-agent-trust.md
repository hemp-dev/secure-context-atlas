---
id: "vuln.ai.inter-agent-trust"
title: "Inter-agent message authenticity and scope failure"
aliases: []
summary: "An agent accepts another agent's message, task or result without authenticating origin, binding it to a workflow and limiting its requested capabilities."
...
genai_mappings: ["ASI07","ASI03"]
applies_to: ["source code","configuration","architecture"]
surfaces: ["agent","queue","workflow"]
languages: ["any"]
frameworks: ["framework-agnostic"]
platforms: ["ai","agent","distributed"]
...
data_flow: {"sources":["agent message, task envelope or queue event"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["next agent, tool call or workflow transition"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Messages lack sender/audience/signature","Task identity is mutable","Agent result is trusted without independent validation"]
configuration_signals: ["Configuration, default or error paths can bypass inter-agent message authenticity and scope failure.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace agent message, task envelope or queue event to next agent, tool call or workflow transition across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for Inter-agent message authenticity and scope failure?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for Inter-agent message authenticity and scope failure; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.inter-agent-trust.positive","eval.inter-agent-trust.negative"]
detector_refs: []
---

# Inter-agent message authenticity and scope failure

An agent accepts another agent's message, task or result without authenticating origin, binding it to a workflow and limiting its requested capabilities.
...
Canonical ID: `vuln.ai.inter-agent-trust`; canonical ontology: `CWE-345`.
