---
id: "vuln.ai.memory-poisoning"
title: "Agent memory and context poisoning"
aliases: []
summary: "Untrusted content is persisted as memory, preference or policy and later treated as trusted context across users, tenants or tasks."
...
data_flow: {"sources":["conversation, document or tool result"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["memory store, retrieval context or agent policy"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Writes lack provenance and owner","Memory is retrieved across tenant boundary","User content can alter system policy"]
configuration_signals: ["Configuration, default or error paths can bypass agent memory and context poisoning.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace conversation, document or tool result to memory store, retrieval context or agent policy across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for Agent memory and context poisoning?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for Agent memory and context poisoning; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.memory-poisoning.positive","eval.memory-poisoning.negative"]
detector_refs: []
---

# Agent memory and context poisoning

Untrusted content is persisted as memory, preference or policy and later treated as trusted context across users, tenants or tasks.
...
Canonical ID: `vuln.ai.memory-poisoning`; canonical ontology: `CWE-1321`.
