---
id: "vuln.ai.tool-output"
title: "Untrusted tool output treated as authority"
aliases: []
summary: "Tool results are parsed or presented to the model without schema, provenance and policy checks, so data can become an instruction or capability grant."
...
genai_mappings: ["ASI02","ASI05"]
applies_to: ["source code","configuration","architecture"]
surfaces: ["agent","tool","MCP"]
languages: ["any"]
...
data_flow: {"sources":["tool result, API response or error"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["model context, next tool call or state mutation"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Raw tool output is concatenated into prompt","Tool result can select another tool","No output size/type/tenant validation"]
configuration_signals: ["Configuration, default or error paths can bypass untrusted tool output treated as authority.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace tool result, API response or error to model context, next tool call or state mutation across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for Untrusted tool output treated as authority?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for Untrusted tool output treated as authority; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.tool-output.positive","eval.tool-output.negative"]
detector_refs: []
---

# Untrusted tool output treated as authority

Tool results are parsed or presented to the model without schema, provenance and policy checks, so data can become an instruction or capability grant.
...
Canonical ID: `vuln.ai.tool-output`; canonical ontology: `CWE-20`.
