---
id: "vuln.injection.ldap"
title: "LDAP filter and distinguished-name injection"
aliases: []
summary: "Untrusted directory input changes an LDAP filter or distinguished name and alters identity or search semantics."
family: "injection"
canonical_cwe: "CWE-90"
related_cwe: []
capec: []
...
data_flow: {"sources":["username, group, filter or directory path"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["LDAP search, bind or modify operation"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Filter string is concatenated","Escaping differs by filter/DN context","Search result is treated as authorization"]
configuration_signals: ["Configuration, default or error paths can bypass ldap filter and distinguished-name injection.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace username, group, filter or directory path to LDAP search, bind or modify operation across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for LDAP filter and distinguished-name injection?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for LDAP filter and distinguished-name injection; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.ldap.positive","eval.ldap.negative"]
detector_refs: []
---

# LDAP filter and distinguished-name injection

Untrusted directory input changes an LDAP filter or distinguished name and alters identity or search semantics.
...
Canonical ID: `vuln.injection.ldap`; canonical ontology: `CWE-90`.
