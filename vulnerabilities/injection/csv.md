---
id: "vuln.injection.csv"
title: "CSV and spreadsheet formula injection"
aliases: []
summary: "Untrusted cell content is exported in a form that a spreadsheet consumer interprets as a formula or external reference."
family: "injection"
canonical_cwe: "CWE-1236"
related_cwe: []
capec: []
...
surfaces: ["export","report","spreadsheet"]
languages: ["any"]
frameworks: ["framework-agnostic"]
platforms: ["web","api","analytics"]
...
data_flow: {"sources":["user name, search term or database field"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["CSV/XLSX export or downstream spreadsheet"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Values beginning with formula markers are emitted unchanged","Export context is not distinguished from display context","Downloaded file is trusted by operators"]
configuration_signals: ["Configuration, default or error paths can bypass csv and spreadsheet formula injection.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace user name, search term or database field to CSV/XLSX export or downstream spreadsheet across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for CSV and spreadsheet formula injection?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for CSV and spreadsheet formula injection; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
related_vulnerabilities: []
references: ["https://cwe.mitre.org/data/definitions/1236.html"]
source_provenance: ["sources/research-notes.md:normalized defensive topic","sources/manifest.yaml:mitre-cwe"]
last_reviewed: "2026-08-08"
maturity: "curated"
priority: "P1"
review_status: "reviewed"
fixture_ids: ["eval.csv.positive","eval.csv.negative"]
detector_refs: []
---

# CSV and spreadsheet formula injection

Untrusted cell content is exported in a form that a spreadsheet consumer interprets as a formula or external reference.
...
Canonical ID: `vuln.injection.csv`; canonical ontology: `CWE-1236`.
