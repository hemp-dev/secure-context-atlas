---
id: "vuln.injection.log"
title: "Log injection and event-integrity failure"
aliases: []
summary: "Untrusted content changes log structure, severity, correlation or audit interpretation because the logging boundary is not encoded or structured safely."
family: "injection"
canonical_cwe: "CWE-117"
related_cwe: []
capec: []
...
surfaces: ["logs","audit","observability"]
languages: ["any"]
frameworks: ["framework-agnostic"]
platforms: ["web","api","cloud","ci-cd"]
...
data_flow: {"sources":["request, username, filename or model output"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["logger, audit sink or SIEM parser"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Newline/control characters are logged raw","User value becomes a field name or severity","Audit events are mixed with free-form content"]
configuration_signals: ["Configuration, default or error paths can bypass log injection and event-integrity failure.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace request, username, filename or model output to logger, audit sink or SIEM parser across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for Log injection and event-integrity failure?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for Log injection and event-integrity failure; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.log.positive","eval.log.negative"]
detector_refs: []
---

# Log injection and event-integrity failure

Untrusted content changes log structure, severity, correlation or audit interpretation because the logging boundary is not encoded or structured safely.
...
Canonical ID: `vuln.injection.log`; canonical ontology: `CWE-117`.
