---
id: "vuln.injection.regex"
title: "Regular-expression denial of service"
aliases: []
summary: "An attacker-controlled pattern or input triggers disproportionate regular-expression work and consumes bounded service resources."
family: "injection"
canonical_cwe: "CWE-1333"
related_cwe: []
capec: []
...
surfaces: ["validation","search","routing"]
languages: ["any"]
...
data_flow: {"sources":["regex pattern, search string or user-defined rule"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["regex engine or repeated validation"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Backtracking expression accepts untrusted input","No match timeout or input bound","Pattern compilation occurs per request"]
configuration_signals: ["Configuration, default or error paths can bypass regular-expression denial of service.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace regex pattern, search string or user-defined rule to regex engine or repeated validation across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for Regular-expression denial of service?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for Regular-expression denial of service; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.regex.positive","eval.regex.negative"]
detector_refs: []
---

# Regular-expression denial of service

An attacker-controlled pattern or input triggers disproportionate regular-expression work and consumes bounded service resources.
...
Canonical ID: `vuln.injection.regex`; canonical ontology: `CWE-1333`.
