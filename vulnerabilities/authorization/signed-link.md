---
id: "vuln.authorization.signed-link"
title: "Signed-link scope and expiry confusion"
aliases: []
summary: "A signed URL or capability token authorizes more resources, actions or time than the issuer intended because scope and lifecycle are incomplete."
family: "authorization-access"
canonical_cwe: "CWE-863"
related_cwe: []
capec: []
owasp_mappings: []
asvs_mappings: []
...
surfaces: ["download","email","API"]
languages: ["any"]
frameworks: ["framework-agnostic"]
platforms: ["web","api","storage"]
...
data_flow: {"sources":["signed link, capability token or share identifier"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["file read, object mutation or privileged action"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Signature covers an ID but not action/tenant","Expiry is checked inconsistently","Token is reusable after revocation or ownership change"]
configuration_signals: ["Configuration, default or error paths can bypass signed-link scope and expiry confusion.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace signed link, capability token or share identifier to file read, object mutation or privileged action across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for Signed-link scope and expiry confusion?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for Signed-link scope and expiry confusion; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.signed-link.positive","eval.signed-link.negative"]
detector_refs: []
---

# Signed-link scope and expiry confusion

A signed URL or capability token authorizes more resources, actions or time than the issuer intended because scope and lifecycle are incomplete.
...
Canonical ID: `vuln.authorization.signed-link`; canonical ontology: `CWE-863`.
