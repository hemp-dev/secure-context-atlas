---
id: "vuln.authorization.service-impersonation"
title: "Service identity and end-user impersonation confusion"
aliases: []
summary: "A service identity is accepted as if it represented an end user, so downstream authorization loses the original principal and tenant context."
...
surfaces: ["service-to-service","queue","API"]
languages: ["any"]
frameworks: ["framework-agnostic"]
platforms: ["microservices","cloud","enterprise"]
...
data_flow: {"sources":["forwarded identity, service token or queue metadata"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["downstream authorization decision"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Caller identity is copied from an unsigned header","Worker uses a broad service account","Impersonation is not time- or scope-bounded"]
configuration_signals: ["Configuration, default or error paths can bypass service identity and end-user impersonation confusion.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace forwarded identity, service token or queue metadata to downstream authorization decision across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for Service identity and end-user impersonation confusion?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for Service identity and end-user impersonation confusion; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.service-impersonation.positive","eval.service-impersonation.negative"]
detector_refs: []
---

# Service identity and end-user impersonation confusion

A service identity is accepted as if it represented an end user, so downstream authorization loses the original principal and tenant context.
...
Canonical ID: `vuln.authorization.service-impersonation`; canonical ontology: `CWE-441`.
