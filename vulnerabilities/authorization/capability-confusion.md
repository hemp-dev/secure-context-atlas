---
id: "vuln.authorization.capability-confusion"
title: "Capability and destination confusion"
aliases: []
summary: "A capability intended for one destination or operation is reused for another, creating a confused deputy across principals or services."
...
surfaces: ["proxy","tool","integration"]
languages: ["any"]
frameworks: ["framework-agnostic"]
platforms: ["api","cloud","ai"]
...
data_flow: {"sources":["destination, tool name or capability token"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["network call, cloud action or tool invocation"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Destination is caller-controlled","Capability token lacks audience","Proxy forwards ambient authority"]
configuration_signals: ["Configuration, default or error paths can bypass capability and destination confusion.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace destination, tool name or capability token to network call, cloud action or tool invocation across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for Capability and destination confusion?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for Capability and destination confusion; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.capability-confusion.positive","eval.capability-confusion.negative"]
detector_refs: []
---

# Capability and destination confusion

A capability intended for one destination or operation is reused for another, creating a confused deputy across principals or services.
...
Canonical ID: `vuln.authorization.capability-confusion`; canonical ontology: `CWE-441`.
