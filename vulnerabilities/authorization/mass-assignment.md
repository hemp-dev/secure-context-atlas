---
id: "vuln.authorization.mass-assignment"
title: "Mass assignment and unsafe property binding"
aliases: []
summary: "A caller-controlled object is bound to fields that change authorization, ownership, pricing or workflow state without an explicit property policy."
family: "authorization-access"
canonical_cwe: "CWE-915"
related_cwe: []
capec: []
owasp_mappings: []
asvs_mappings: []
...
data_flow: {"sources":["request body or patch document"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["model update or security-sensitive property"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Generic deserializer accepts privileged fields","Allow and deny lists are not versioned","Nested objects bypass top-level property policy"]
configuration_signals: ["Configuration, default or error paths can bypass mass assignment and unsafe property binding.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace request body or patch document to model update or security-sensitive property across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for Mass assignment and unsafe property binding?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for Mass assignment and unsafe property binding; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.mass-assignment.positive","eval.mass-assignment.negative"]
detector_refs: []
---

# Mass assignment and unsafe property binding

A caller-controlled object is bound to fields that change authorization, ownership, pricing or workflow state without an explicit property policy.
...
Canonical ID: `vuln.authorization.mass-assignment`; canonical ontology: `CWE-915`.
