---
id: "vuln.authorization.webhook-signature"
title: "Webhook signature verification bypass"
aliases: []
summary: "A webhook or callback is treated as trusted without authenticating its origin, binding the signature to the exact body and preventing replay."
...
surfaces: ["webhook","API","queue"]
languages: ["any"]
frameworks: ["framework-agnostic"]
platforms: ["api","integration","supply-chain"]
...
data_flow: {"sources":["webhook body, headers and timestamp"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["state change, job enqueue or downstream API call"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Signature checked after parsing or normalization","Timestamp/nonce is not bounded","Fallback path accepts unsigned callbacks"]
configuration_signals: ["Configuration, default or error paths can bypass webhook signature verification bypass.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace webhook body, headers and timestamp to state change, job enqueue or downstream API call across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for Webhook signature verification bypass?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for Webhook signature verification bypass; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.webhook-signature.positive","eval.webhook-signature.negative"]
detector_refs: []
---

# Webhook signature verification bypass

A webhook or callback is treated as trusted without authenticating its origin, binding the signature to the exact body and preventing replay.
...
Canonical ID: `vuln.authorization.webhook-signature`; canonical ontology: `CWE-345`.
