---
id: "vuln.authentication.password-storage"
title: "Weak password hashing and credential storage"
aliases: []
summary: "Passwords or recovery secrets are stored with a fast, unsalted or misconfigured derivation that does not provide an appropriate offline-cracking cost."
family: "authentication-identity"
canonical_cwe: "CWE-916"
related_cwe: []
capec: []
owasp_mappings: []
asvs_mappings: []
...
surfaces: ["login","database","migration"]
languages: ["any"]
...
data_flow: {"sources":["password input or credential record"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["credential verifier or database"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Plain hash or reversible encryption","Work factor is static and obsolete","Pepper is hardcoded or logged"]
configuration_signals: ["Configuration, default or error paths can bypass weak password hashing and credential storage.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace password input or credential record to credential verifier or database across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for Weak password hashing and credential storage?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for Weak password hashing and credential storage; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.password-storage.positive","eval.password-storage.negative"]
detector_refs: []
---

# Weak password hashing and credential storage

Passwords or recovery secrets are stored with a fast, unsalted or misconfigured derivation that does not provide an appropriate offline-cracking cost.
...
Canonical ID: `vuln.authentication.password-storage`; canonical ontology: `CWE-916`.
