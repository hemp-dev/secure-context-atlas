---
id: "vuln.authentication.password-reset"
title: "Password-reset token lifecycle failure"
aliases: []
summary: "A password-reset flow accepts a token that is predictable, reusable, unscoped, overlong-lived or not bound to the intended account and purpose."
family: "authentication-identity"
canonical_cwe: "CWE-640"
related_cwe: []
capec: []
owasp_mappings: []
asvs_mappings: []
...
data_flow: {"sources":["reset token, email link or recovery request"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["credential change or account recovery"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Token is stored or logged in plaintext","Reset does not rotate sessions","Account selection is inferred from client input"]
configuration_signals: ["Configuration, default or error paths can bypass password-reset token lifecycle failure.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace reset token, email link or recovery request to credential change or account recovery across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for Password-reset token lifecycle failure?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for Password-reset token lifecycle failure; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.password-reset.positive","eval.password-reset.negative"]
detector_refs: []
---

# Password-reset token lifecycle failure

A password-reset flow accepts a token that is predictable, reusable, unscoped, overlong-lived or not bound to the intended account and purpose.
...
Canonical ID: `vuln.authentication.password-reset`; canonical ontology: `CWE-640`.
