---
id: "vuln.configuration.fail-open"
title: "Fail-open exceptional conditions and unsafe defaults"
aliases: ["fail open","unsafe default","exception bypass"]
summary: "A timeout, parse error, missing configuration, dependency failure or unexpected state grants access or continues a sensitive operation instead of failing safely."
family: "configuration-deployment"
canonical_cwe: "CWE-636"
related_cwe: ["CWE-693"]
capec: []
owasp_mappings: []
asvs_mappings: []
wstg_mappings: []
masvs_mappings: []
api_security_mappings: []
genai_mappings: []
applies_to: ["source code","configuration","architecture"]
surfaces: ["auth","network","parser","feature flag","deployment"]
languages: ["any"]
frameworks: ["framework-agnostic"]
platforms: ["server","cloud","container","mobile"]
preconditions: ["An untrusted or lower-trust input reaches the described boundary.","A security control is expected at or before the sink but is absent, incomplete or applied on only one path."]
trust_boundaries: ["untrusted input to application","application to privileged/resource sink"]
data_flow: {"sources":["exception, timeout, absent config or dependency response"],"transformations":["decode, normalize or parse input","application-specific routing"],"controls":["validation or policy control if present"],"sinks":["fallback branch, feature flag or authorization decision"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Catch-all exception returns success","Missing policy treated as allow","Default credentials/debug mode","Circuit breaker bypasses authorization or audit"]
configuration_signals: ["Configuration flags, defaults or error paths can bypass fail-open exceptional conditions and unsafe defaults.","Inspect deployment, proxy, identity and resource-limit settings that affect the control."]
architecture_signals: ["Trace exception, timeout, absent config or dependency response to fallback branch, feature flag or authorization decision across synchronous, asynchronous, alternate-protocol and provider boundaries."]
audit_questions: ["What exact source and sink are reachable?","Which control should run before the sink?","Do alternate protocols, retries or error paths differ?"]
safe_verification: ["Use a local or staging fixture with synthetic data, test identities and a unique canary; keep egress disabled.","Assert the expected denial or safe encoding and inspect only test output; do not use real credentials, persistence or destructive state."]
false_positives: ["The named library or endpoint is only a signal; confirm reachability and the exact sink."]
impact: ["security boundary bypass","confidentiality/integrity/availability impact"]
severity_factors: ["Severity depends on the asset, privilege and blast radius."]
exploitability_factors: ["Reachability and a controllable input are required; use test fixtures only."]
remediation: ["Enforce the control at the sink-facing service boundary.","Make policy explicit, deny by default and cover alternate/async paths.","Add telemetry and bounded failure behaviour."]
secure_patterns: ["Typed inputs and explicit policy checks.","Least privilege, isolation and bounded resource use."]
regression_tests: ["Add a local or staging negative test using a canary and assert the control blocks the unsafe sink."]
related_vulnerabilities: []
references: ["https://cwe.mitre.org/data/definitions/636.html","https://owasp.org/Top10/2025/A10_2025-Mishandling_of_Exceptional_Conditions/"]
source_provenance: ["sources/manifest.yaml:mitre-cwe","sources/manifest.yaml:owasp-asvs"]
last_reviewed: "2026-08-07"
---

# Fail-open exceptional conditions and unsafe defaults

Define fail-closed behaviour per operation, distinguish availability fallback from security decision, make defaults explicit, alert on policy/config errors and add tests for timeouts, malformed inputs and missing dependencies.

## Audit notes

Use the source/transform/control/sink model above. Report only evidence-backed flows and keep verification inside the safe boundary.

## Retrieval

Canonical ID: `vuln.configuration.fail-open`; canonical ontology entry: `CWE-636`.
