---
id: "vuln.configuration.debug"
title: "Debug and diagnostic mode exposed in deployment"
aliases: []
summary: "Diagnostic features expose source, state, interactive controls or verbose data outside a controlled development boundary."
family: "configuration-deployment"
canonical_cwe: "CWE-489"
related_cwe: []
capec: []
owasp_mappings: []
asvs_mappings: []
...
surfaces: ["web","API","admin"]
languages: ["any"]
frameworks: ["framework-agnostic"]
platforms: ["web","cloud","container"]
...
data_flow: {"sources":["deployment flag, debug route or error state"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["diagnostic endpoint, console or response"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Debug enabled by default","Non-production check trusts hostname","Admin console is reachable from public route"]
configuration_signals: ["Configuration, default or error paths can bypass debug and diagnostic mode exposed in deployment.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace deployment flag, debug route or error state to diagnostic endpoint, console or response across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for Debug and diagnostic mode exposed in deployment?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for Debug and diagnostic mode exposed in deployment; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.debug.positive","eval.debug.negative"]
detector_refs: []
---

# Debug and diagnostic mode exposed in deployment

Diagnostic features expose source, state, interactive controls or verbose data outside a controlled development boundary.
...
Canonical ID: `vuln.configuration.debug`; canonical ontology: `CWE-489`.
