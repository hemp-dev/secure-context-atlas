---
id: "vuln.cloud.k8s-service-account"
title: "Kubernetes service-account and workload identity overreach"
aliases: []
summary: "A workload receives a service account or cloud identity with broader cluster, namespace or provider permissions than its task requires."
...
surfaces: ["Kubernetes","pod","cluster API"]
languages: ["any"]
frameworks: ["framework-agnostic"]
platforms: ["kubernetes","cloud","container"]
...
data_flow: {"sources":["pod spec, service account token or workload identity binding"],"transformations":["decode, normalize or parse input","application-specific routing or policy translation"],"controls":["validation, authorization, provenance or budget control if present"],"sinks":["Kubernetes/cloud control-plane API"],"authorization_points":["the decision point immediately before the sink"]}
code_signals: ["Default service account mounted","Wildcard verbs/resources","Namespace identity mapped to broad cloud role"]
configuration_signals: ["Configuration, default or error paths can bypass kubernetes service-account and workload identity overreach.","Review identity, boundary, limit and failure settings for every alternate path."]
architecture_signals: ["Trace pod spec, service account token or workload identity binding to Kubernetes/cloud control-plane API across synchronous, asynchronous and provider boundaries.","Check whether a gateway-only control is assumed to protect internal or worker paths."]
audit_questions: ["What exact source and sink are reachable for Kubernetes service-account and workload identity overreach?","Which control must run before the sink and where is its decision evidence?","Do retries, alternate protocols, error paths and tenant changes preserve the same control?"]
...
regression_tests: ["Create a local or staging synthetic fixture for Kubernetes service-account and workload identity overreach; assert the control blocks or safely contains the unsafe path and records a redacted decision."]
...
fixture_ids: ["eval.k8s-service-account.positive","eval.k8s-service-account.negative"]
detector_refs: []
---

# Kubernetes service-account and workload identity overreach

A workload receives a service account or cloud identity with broader cluster, namespace or provider permissions than its task requires.
...
Canonical ID: `vuln.cloud.k8s-service-account`; canonical ontology: `CWE-269`.
